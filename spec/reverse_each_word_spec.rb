...
describe '#reverse_each_word' do
 ...
 expect |:sentence1| #(reverse_each_word.map.each(sentence1)).to eq("olleH ,ereht dna woh era ?uoy")
  end
  ...
  it 'reverses all the words in another string without reversing the order of the words' do
   expect|:sentence2| #(reverse_each_word.map.each(sentence2)).to eq("iH ,niaga tsuj gnikam erus s'ti !desrever")
  end
